...
- **2026-09-02 — Focus 4 (current cycle, latest): Windowing/Mission Control.** Window Overview is now app-neutral and spatial: each current window is represented by a geometry-only mini-map of its real effective frame, active/maximized/minimized state is visible, selecting a card restores/activates only that existing window, and `Restore All` restores minimized current windows without silently launching the optional Vibe workspace. Product head `d11436e` (feature started at `9eb9f9a`); Apple Build, Pages and Apple Integration Smoke all passed on **attempt 1**, including smoke evidence upload. Real spatial readability and selection feel through RayNeo remain `NEEDS PHYSICAL TEST`.
- **Next rotation focus: 5 — Phone controller ergonomics.** Preserve the full-screen trackpad with only Keyboard + More visible; choose the highest-impact unfinished contextual-control, one-handed, clipboard/app-switching or keyboard-access issue without restoring permanent control rails.
