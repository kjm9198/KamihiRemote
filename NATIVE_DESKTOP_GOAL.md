...
- **2026-09-02 — Focus 3 (current cycle): Trackpad/pointer.** Retuned only the stock Balanced full-screen-trackpad profile from sensitivity/acceleration `1.20/1.00` to `1.12/0.82`, reducing low-speed overshoot while preserving the existing velocity acceleration and adaptive smoothing. A versioned migration applies only when persisted values are still the old stock defaults; any custom pointer tuning is preserved exactly. Feature head `d270fc5`; Pages, Apple Build and Apple Integration Smoke all passed on **attempt 1**, including the simulator-host smoke and evidence upload. The Desktop Lab artifact was visually inspected: the 1920×1080 external canvas/calibration remains intact and the phone controller is still the simplified full-trackpad surface with only Keyboard and More. Real pointer latency and the final Balanced feel on iPhone + RayNeo remain `NEEDS PHYSICAL TEST`.
- **2026-09-02 — Focus 4 (current cycle): Windowing/Mission Control.** Window Overview is now app-neutral and spatial: each current window is represented by a geometry-only mini-map of its real effective frame, active/maximized/minimized state is visible, selecting a card restores/activates only that existing window, and `Restore All` restores minimized current windows without silently launching the optional Vibe workspace. Product head `d11436e` (feature started at `9eb9f9a`); Apple Build, Pages and Apple Integration Smoke all passed on **attempt 1**, including smoke evidence upload. Real spatial readability and selection feel through RayNeo remain `NEEDS PHYSICAL TEST`.
- **2026-09-02 — Focus 5 (current cycle, latest): Phone controller ergonomics.** Locked the new normal-controller contract around the full-screen trackpad: only Keyboard + More remain permanently visible, legacy preview/control-rail surfaces cannot return silently, and keyboard focus remains tied to the explicitly captured active desktop window. Product/CI heads `045b43d` → `e8172fb`; Apple Build, Pages and Apple Integration Smoke all passed on **attempt 1**. Physical one-handed feel and hardware/Bluetooth keyboard behavior remain `NEEDS PHYSICAL TEST`.
- **2026-09-02 — Focus 6 (current cycle, latest): Browser/web-app quality.** Desktop Browser now converts HTTP(S) navigations that request a separate WebKit window (`target=_blank` / `window.open`) into normal retained Kamihi browser tabs, rather than silently dropping the navigation or spawning an unmanaged window. The existing persistent tab/session/bookmark/history model and default WebKit website data store remain unchanged. Feature head `813fc6d`; iOS build, macOS regression build, Pages and Apple Integration Smoke all passed on **attempt 1**, including the controller contract gate and smoke-evidence upload. The Desktop Lab artifact was visually inspected as clean/Desktop-first at 1920×1080 with the simplified phone trackpad controller intact. Real OAuth/site-specific popup behavior and login continuity remain `NEEDS PHYSICAL TEST`.
- **2026-09-02 — Focus 5 (newest pass): Phone controller ergonomics.** Reclaimed the normal iPhone controller as an uninterrupted trackpad surface by removing the persistent active-window/status block and deleting the dormant preview/header/controller-rail layout from `DesktopControllerView`. Only the 44 pt Keyboard + More controls remain visible; active-window and display status now live inside More alongside apps, windows, commands, trackpad settings, capture and Continue on iPhone. Keyboard focus isolation is preserved. Feature head `17f58ed`; Pages/deploy, Apple Build and Apple Integration Smoke all passed on **attempt 1**, including the trackpad contract, simulator-host smoke and evidence upload. Desktop Lab was visually inspected at 1920×1080 and shows only Keyboard + More over the phone trackpad. Physical one-handed reach and hardware/Bluetooth keyboard feel remain `NEEDS PHYSICAL TEST`.
- **2026-09-03 — Focus 10 (latest readiness pass): Keyboard/accessibility/polish.** The external-desktop software pointer now follows iOS `Increase Contrast` and `Differentiate Without Color`: Kamihi Dot, Arrow, Precision, text, drag, resize and busy states strengthen their black/white silhouette and non-accessibility pointer styles receive a modest visibility scale increase without adding an idle display-link loop. Product head `fa1bad27`; Apple Build and Apple Integration Smoke/Desktop Simulator Smoke both passed on **attempt 1**. The exact-SHA Desktop Lab artifact was visually inspected: 1920×1080 full-canvas calibration remained intact, the center pointer stayed clearly readable against the mixed desktop background, and the iPhone remained a full trackpad with only Keyboard + More. Real glasses visibility, pointer latency and accessibility preference feel remain `NEEDS PHYSICAL TEST`.
- **Next rotation focus: 1 — RayNeo display fidelity.** Choose the highest-impact unfinished software-testable external-display issue without repeating calibration/readout work; keep actual resolution/refresh, overscan/readability and USB-C behavior marked `NEEDS PHYSICAL TEST` until verified on real hardware.
