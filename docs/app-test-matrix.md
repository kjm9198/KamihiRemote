...
| Hardware keyboard / mouse | N/A | PARTIAL | PARTIAL | PARTIAL | PARTIAL | N/A | Public GameController/UIKit bridge now covers transport-independent mouse/keyboard detection, physical pointer motion, isolated left-click vs direct title-bar drag/edge resize, right-click, vertical/horizontal wheel routing, MX Master-style first two auxiliary buttons as Browser Back/Forward, and direct hardware-keyboard text capture for focused Notes/Documents/Sheets/WebView fields. Software CI can verify compile/smoke only; Bluetooth vs USB-C transport, MX Master DPI/scroll feel, side-button ordering, and real hardware focus remain NEEDS PHYSICAL TEST. |
