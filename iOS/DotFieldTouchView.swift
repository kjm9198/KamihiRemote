...
struct DotFieldTouchView: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: reduceMotion ? 1.0 / 30.0 : 1.0 / 60.0, paused: false)) { timeline in
    ...
    private func drawField(for finger: TouchAnimationFinger, in context: inout GraphicsContext, size: CGSize, time: TimeInterval) {
        let count = reduceMotion ? 90 : (state.fingerCount >= 3 ? 140 : 220)
        ...
        for index in 0..<count {
            ...
            let theta = hash2 * .pi * 2 + time * (0.15 + hash3 * 0.25)
