...
- **2026-09-02 — Focus 9 (current cycle, latest): Performance/energy.** External Desktop now observes the existing live iOS power/thermal monitor and suppresses purely decorative snap-preview/launcher animation plus the large launcher shadow when Low Power Mode is on or thermal pressure reaches serious/critical; Reduce Motion remains an equal trigger. Pointer movement, window manipulation and WebKit behavior are deliberately not throttled, so responsiveness is preserved. Feature head `2fe7600`; Pages/deploy is green and Apple Build/Apple Integration Smoke are running on their original attempt 1 at the time of this note. Physical battery/thermal benefit remains `NEEDS PHYSICAL TEST`.
- **Next rotation focus: 10 — keyboard/accessibility/polish.** Choose a high-impact unfinished hardware-shortcut, VoiceOver, Dynamic Type, contrast, large-pointer or visual-QA issue without repeating the prior large-text startup-profile work.
