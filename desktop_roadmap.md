...
| 43 | WebView sleeping | PARTIAL | Focus 9 stops rendering minimized window content entirely and unloads inactive Browser/ChatGPT/YouTube content during battery/thermal conservation. The retained Browser warm pool is capped separately; long-idle policy and physical memory-pressure profiling remain TODO. |
...
2. Finish Photos permission flow and richer Files organization/export; desktop capture/share is now implemented and awaits physical fidelity checks.
3. Surface recovery/display-health UI and validate abnormal-termination restore behavior.
4. Add browser downloads/reader/pinning polish and stronger memory-pressure/long-idle retained-tab recovery behavior.
...
- **2026-09-02 — Focus 9 (current cycle, latest): Performance/energy.** External Desktop now observes the existing live iOS power/thermal monitor and suppresses purely decorative snap-preview/launcher animation plus the large launcher shadow when Low Power Mode is on or thermal pressure reaches serious/critical; Reduce Motion remains an equal trigger. Pointer movement, window manipulation and WebKit behavior are deliberately not throttled, so responsiveness is preserved. Feature head `2fe7600`; successor head `cb9eb394` is green across Apple Build, Pages/deploy and Apple Integration Smoke, with smoke passing on attempt 1. Physical battery/thermal benefit remains `NEEDS PHYSICAL TEST`.
- **2026-09-02 — Focus 10 (current cycle, latest): Keyboard/accessibility/polish.** Added a discoverable Keyboard & Gestures guide to the Desktop Command Palette, covering the actual phone trackpad gestures, window cycling, keyboard entry, Precision Mode and secure Continue-on-iPhone flow. Command rows now expose cleaner combined VoiceOver semantics and decorative glyphs are hidden from accessibility. The guide uses semantic Dynamic Type styles rather than fixed text sizes and does not advertise unverified hardware shortcuts. Feature head `ebd5b3d`: Apple Build and Pages/deploy passed; Apple Integration Smoke’s real iPhone+Mac integration step passed on **attempt 1** and smoke evidence uploaded successfully. The Desktop Lab artifact was visually inspected as clean/Desktop-first with no Remote-for-Mac compartment or obvious clipping/overlap regression. Hardware-keyboard bindings and real VoiceOver/Dynamic Type feel remain physical/device follow-up.
- **2026-09-02 — Focus 1 (current cycle, latest): RayNeo Air 4 Pro external-display fidelity.** Added an explicit high-contrast external calibration guide to the actual Desktop canvas: physical outer border, four corner L-marks, quarter/center grid, persisted safe-margin outline, plus the exact resolution and maximum refresh ceiling reported by iOS. It appears briefly on canvas/reconnect metric changes and stays visible while a nonzero overscan margin is configured. It never switches display modes or claims iOS can force 120 Hz. Real RayNeo cropping, 1920×1080 negotiation, actual refresh, readability and USB-C reconnect remain `NEEDS PHYSICAL TEST`.
- **2026-09-02 — Focus 4 (current cycle, latest): Spatial Window Overview.** Replaced Window Overview's generic placeholder with a geometry-only mini-map of each current window's real effective frame and state, and removed the misleading `Tile All` behavior that silently launched the optional Vibe workspace. `Restore All` now restores only minimized current windows; selecting a card restores/activates that existing window. Product head `d11436e` (feature started at `9eb9f9a`); Apple Build, Pages and Apple Integration Smoke passed on **attempt 1**, including smoke evidence upload. Real spatial readability and selection feel through RayNeo remain `NEEDS PHYSICAL TEST`.
- **Next rotation focus: 5 — Phone controller ergonomics.** Preserve the full-screen trackpad with only Keyboard + More visible; choose a new high-impact contextual/keyboard/one-handed improvement without bringing back permanent control rails.
