...
| 27 | Physical keyboard support | PARTIAL | iOS keyboard support and several SwiftUI keyboardShortcut bindings exist; `180ef536` adds direct Command-1 through Command-6 startup-profile selection for Clean Desktop, Resume, Work, Browse, Media and optional Vibe. Full desktop focus plus close/cycle/layout routing and real Bluetooth/hardware-keyboard validation remain TODO. |
...
| 64 | Accessibility mode | PARTIAL | Reduce Motion and transparency-aware shell behavior already exist; `ebd5b3d` added the VoiceOver-friendly Keyboard & Gestures guide. `180ef536` further improves the normal startup selector with accessibility-size single-column cards and stacked feature copy, VoiceOver header/selected semantics, Differentiate Without Color cues, exact shortcut hints, preserved Reduce Transparency behavior, and a 44-point Desktop Lab control. Broader contrast/focus-navigation audit and preset profiles remain TODO. |
...
| 75 | Hardware keyboard shortcut layer | PARTIAL | Existing Cmd-K, launcher/settings/save shortcuts remain; `180ef536` adds Command-1 through Command-6 for the six Desktop startup profiles and exposes those shortcuts in accessibility-size/non-color cues. Active-window close/cycle/layout bindings and complete focus routing remain TODO. |
...
| 89 | Keyboard command cheat sheet | PARTIAL | `ebd5b3d` adds a discoverable Keyboard & Gestures sheet from the Command Palette. `180ef536` makes startup profiles themselves keyboard-discoverable: accessibility hints announce Command-1 through Command-6 and accessibility-size/Differentiate Without Color layouts show the corresponding shortcut cue. Exact active-desktop close/cycle/layout inventory remains TODO. |
...
6. Complete active-desktop hardware-keyboard close/cycle/layout shortcuts and focus routing; startup-profile Command-1 through Command-6 shortcuts and the in-app Keyboard & Gestures guide are already present.
...
- **2026-09-02 — Focus 9 (current cycle): Performance/energy and Browser renderer lifecycle.** Feature head `e4ad849` keeps the active Browser tab live but sheds every inactive retained `WKWebView` when iOS issues a memory warning or the app enters the background. Under iOS Low Power Mode, the MRU warm Browser pool is capped at two renderers instead of six. Persistent tab URL/title metadata and WebKit website data remain outside that renderer lifetime, so tabs can be lazily recreated without Kamihi inspecting or storing credentials. Apple Build and the trackpad-first contract passed on attempt 1; Apple Integration Smoke's real simulator+host step and evidence upload also passed on attempt 1, and the Desktop Lab artifact was visually inspected as clean/Desktop-first. Real memory-pressure recovery, background/foreground continuity, battery benefit and long-session stability remain `NEEDS PHYSICAL TEST`.
- **2026-09-02 — Focus 10 (current cycle, latest): Keyboard/accessibility/polish.** Feature head `180ef536` makes the normal Desktop startup selector substantially more accessible without changing its Desktop-first behavior: Command-1 through Command-6 select Clean Desktop, Resume, Work, Browse, Media and optional Vibe; accessibility-size layouts use a one-column profile grid and stack the featured Desktop card; VoiceOver gets header and selected-state semantics plus exact shortcut hints; Differentiate Without Color gets a distinct selected symbol/shortcut cue; Reduce Transparency remains respected; and the DEBUG Desktop Lab button has a 44-point minimum target with Command-Shift-D. Apple Build passed on **attempt 1** including the iOS compile and legacy host regression. Apple Integration Smoke passed the trackpad-first contract, real simulator+host step and evidence upload on **attempt 1** with no rerun/harness workaround. Desktop Lab and mode-chooser screenshots were visually inspected as clean/Desktop-first with no Remote-for-Mac compartment or obvious clipping regression. Real Bluetooth/hardware-keyboard behavior and interactive VoiceOver/Dynamic Type/Differentiate Without Color/Reduce Transparency validation remain `NEEDS PHYSICAL TEST`. **Next rotation: Focus 1 — RayNeo Air 4 Pro external-display fidelity; choose a new unfinished issue such as automatic recommended-scale application or reconnect/display-health feedback rather than repeating calibration-guide work.**
