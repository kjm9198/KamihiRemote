...
| 15 | Native Kamihi Notes | PARTIAL | Local autosaving notes now keep phone-editor changes durable, update edit timestamps consistently, sort/search by recency/content, expose delete controls, and use semantic iOS styling; full external-display pointer/keyboard routing still remains. |
...
- **2026-09-01 — Focus 8 (current cycle): Native desktop apps.** Improved native Notes without revisiting the prior Files/PDF slice: durable phone-editor persistence, consistent edit timestamps, recency ordering, title/body search, deletion affordances, semantic system styling, and clearer accessibility labels. Feature code head `c23eda9`; Apple Build iOS and macOS compile steps passed on attempt 1 during PR validation. External-display native pointer/keyboard routing remains unfinished and is not claimed verified.
- **Next rotation focus: 9 — performance/energy.** Choose a different unfinished issue from the prior WebView-sleeping change; prioritize measurable idle rendering/memory cleanup or long-session stability without repeating the same conservation policy work.